...
def all_winter_holiday_supplies(holiday_hash)
  ...
  holiday_hash.collect.do |season, holiday_name_hash|
    if season == :winter
      return holiday_name_hash.values.flatten
    end
  end
end
...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, holiday_name_hash|
    ...
    holiday_name_hash.each do |holiday_name, supplies|
      holiday_final = holiday_name.to_s.split("_").each{|holiday| holiday.capitalize!}.join(' ')
      holiday_supplies = " #{holiday_final}: #{supplies.join', '}"
